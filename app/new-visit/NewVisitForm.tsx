    ...
    try {
      ...
      if (isNewRestaurant) {
        ...
        const { data, error } = await supabase.from('restaurants').insert({
          name: (newRestaurant.name || '').trim().slice(0, 100),
          cuisine_type: (newRestaurant.cuisine_type || '').trim().slice(0, 50) || null,
          price_range: !isNaN(parsedPrice) ? parsedPrice : null,
          address: (newRestaurant.address || '').trim().slice(0, 255) || null,
          notes: (newRestaurant.notes || '').trim().slice(0, 1000) || null,
        }).select('id').single()
      ...
      const { data: visit, error: visitError } = await supabase.from('visits').insert({
        ...
        occasion: (occasion || '').trim().slice(0, 50) || null,
      ...
      for (const dish of dishes.filter(d => d.name.trim())) {
        ...
        const { data: dishData } = await supabase.from('dishes').insert({
          visit_id: visit.id,
          name: (dish.name || '').trim().slice(0, 100),
          category: (dish.category || '').trim().slice(0, 50) || null,
          rating: dish.rating,
