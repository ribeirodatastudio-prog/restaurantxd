          ...
          <div className="flex flex-wrap gap-2">
            {people.map(p => (
              <button key={p.id} type="button"
                ...
                className={`tag cursor-pointer transition-colors focus-visible:ring-2 focus-visible:ring-[#c9a96e] focus-visible:ring-offset-1 focus-visible:ring-offset-[#1a1815] focus:outline-none ${selectedPeople.includes(p.id) ? 'tag-accent' : ''}`}
          ...
          {dishes.map((dish, i) => (
            <div key={i} style={{ border: '1px solid #2a2622', borderRadius: 8 }}>
              <div className="flex items-center gap-2 p-3 cursor-pointer"
                ...
                />
                <button type="button" aria-expanded={dishExpanded === i} className="focus-visible:ring-2 focus-visible:ring-[#c9a96e] focus-visible:ring-offset-1 focus-visible:ring-offset-[#1a1815] focus:outline-none rounded-sm" aria-label={dishExpanded === i ? "Recolher detalhes do prato" : "Expandir detalhes do prato"} onClick={e => { e.stopPropagation(); setDishExpanded(dishExpanded === i ? null : i) }}>
                  {dishExpanded === i ? <ChevronUp size={16} style={{ color: '#8a8278' }} /> : <ChevronDown size={16} style={{ color: '#8a8278' }} />}
                </button>
                <button type="button" className="focus-visible:ring-2 focus-visible:ring-[#c9a96e] focus-visible:ring-offset-1 focus-visible:ring-offset-[#1a1815] focus:outline-none rounded-sm" aria-label="Remover prato" onClick={e => { e.stopPropagation(); removeDish(i) }}>
                  ...
                  <div className="flex gap-2">
                    ...
                    {[true, false].map(v => (
                      <button key={String(v)} type="button"
                        ...
                        className={`tag cursor-pointer focus-visible:ring-2 focus-visible:ring-[#c9a96e] focus-visible:ring-offset-1 focus-visible:ring-offset-[#1a1815] focus:outline-none ${dish.would_order_again === v ? 'tag-accent' : ''}`}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {people.map(p => (
                          <button key={p.id} type="button"
                            ...
                            className={`tag cursor-pointer focus-visible:ring-2 focus-visible:ring-[#c9a96e] focus-visible:ring-offset-1 focus-visible:ring-offset-[#1a1815] focus:outline-none ${dish.peopleIds.includes(p.id) ? 'tag-accent' : ''}`}
                            style={{ padding: '4px 12px', fontSize: 12 }}
