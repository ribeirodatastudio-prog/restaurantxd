...
async function getStats() {
  // Optimize Supabase query payload by selecting only necessary fields and using count
  ...
  // Optimize data aggregation using a single standard for-loop pass to minimize intermediate allocations
  let ratedCount = 0
  let sumRating = 0
  const cuisineCount: Record<string, number> = {}
  const topVisitsRaw: any[] = []

  for (let i = 0; i < v.length; i++) {
    const x = v[i]
    if (x.rating_overall) {
      sumRating += x.rating_overall
      ratedCount++
      topVisitsRaw.push(x)
    }

    const rel = x.restaurant
    const restaurant = Array.isArray(rel) ? rel[0] : rel
    const c = restaurant?.cuisine_type ?? 'Outra'
    cuisineCount[c] = (cuisineCount[c] ?? 0) + 1
  }

  const avgRating = ratedCount > 0 ? sumRating / ratedCount : null

  // Top rated visits
  const topVisits = topVisitsRaw
