...
async function getStats() {
  // ⚡ Bolt Optimization:
  // - Explicitly select only the fields needed for stats calculation.
  // - Use { count: 'exact', head: true } to avoid fetching restaurant records just for the total count.
  const [{ data: visits }, { data: dishes }, { count: restaurantsCount }] = await Promise.all([
    ...
    supabase.from('dishes').select('id, would_order_again'),
