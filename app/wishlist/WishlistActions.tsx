  ...
  return (
    <form className="card mt-2" onSubmit={save}>
      ...
      <div className="flex flex-col gap-2">
        <input aria-label="Nome do lugar" placeholder="Nome *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
        <div className="grid grid-cols-2 gap-2">
          <select aria-label="Tipo de cozinha" value={form.cuisine_type} onChange={e => setForm({ ...form, cuisine_type: e.target.value })}>
            <option value="">Cozinha</option>
            {CUISINE_TYPES.map(c => <option key={c}>{c}</option>)}
          </select>
          <select aria-label="Faixa de preço" value={form.price_range} onChange={e => setForm({ ...form, price_range: e.target.value })}>
            <option value="">Preço</option>
            <option value="1">R$</option><option value="2">R$R$</option>
            <option value="3">R$R$R$</option><option value="4">R$R$R$R$</option>
          </select>
        </div>
        <input aria-label="Endereço do lugar" placeholder="Endereço" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} />
        <textarea aria-label="Notas sobre o lugar" placeholder="Notas" rows={2} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
